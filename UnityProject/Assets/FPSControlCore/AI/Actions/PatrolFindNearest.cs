...
public class PatrolFindNearest : Action
{
    ...
    public override ActionResult Execute(Agent agent, float deltaTime)
    {
		...
		agent.actionContext.AddContextItem<int>("patroldirection", 1);
		agent.actionContext.AddContextItem<int>("patrolnumber", tMinPatrol);
		agent.actionContext.AddContextItem<Vector3>("patrolposition", agent.actionContext.GetContextItem<Vector3>("patrol" + tMinPatrol));
