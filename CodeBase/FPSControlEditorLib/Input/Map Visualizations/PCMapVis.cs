...
{
    public class PCMapVis : EditorControlMapVis<DesktopControlMap>, IEditorControlMapVis
    {
        const string PC = "PC";
        Texture outline, capsFill, escFill, funcFill, shiftFill, spaceFill, tabFill, keyFill;
        Texture mouseOutline, mouseLeft, mouseRight, mouseMotion;

        public ControlMap Map { get { return map; } }

        public PCMapVis(DesktopControlMap focusedMap, System.Action RepaintCallback)
            : base(focusedMap, RepaintCallback)
        {
            LoadAssets();
        }
        ...
        {
            outline = AssetLoader.LoadPNG(GENERAL_DIR + PC, "outline");
            capsFill = AssetLoader.LoadPNG(GENERAL_DIR + PC, "pc-caps");
            escFill = AssetLoader.LoadPNG(GENERAL_DIR + PC, "pc-small");
            funcFill = AssetLoader.LoadPNG(GENERAL_DIR + PC, "pc-func");
            shiftFill = AssetLoader.LoadPNG(GENERAL_DIR + PC, "pc-shift");
            spaceFill = AssetLoader.LoadPNG(GENERAL_DIR + PC, "pc-space");
            tabFill = AssetLoader.LoadPNG(GENERAL_DIR + PC, "pc-tab");
            keyFill = AssetLoader.LoadPNG(GENERAL_DIR + PC, "pc-key");

            mouseOutline = AssetLoader.LoadPNG(GENERAL_DIR + PC, "mouse-outline");
            mouseLeft = AssetLoader.LoadPNG(GENERAL_DIR + PC, "mouse-left");
            mouseRight = AssetLoader.LoadPNG(GENERAL_DIR + PC, "mouse-right");
            mouseMotion = AssetLoader.LoadPNG(GENERAL_DIR + PC, "mouse-arrows");
        }
        ...
        public void Draw()
        {
            ...
            DrawFill(new Vector2(8, 167), funcFill, KeyCode.LeftControl);
            DrawFill(new Vector2(67, 167), funcFill, KeyCode.LeftAlt);
            //DrawFill(new Vector2(97, 167), funcFill, KeyCode.LeftApple);
            DrawFill(new Vector2(97, 167), spaceFill, KeyCode.Space);
            //DrawFill(new Vector2(285, 167), funcFill, KeyCode.RightApple);
