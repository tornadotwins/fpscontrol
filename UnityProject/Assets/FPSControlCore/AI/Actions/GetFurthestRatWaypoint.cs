...
public class GetFurthestRatWaypoint : Action
{
    public GetFurthestRatWaypoint()
    {
        actionName = "GetFurthestRatWaypoint";
    }

    public override ActionResult Execute(Agent agent, float deltaTime)
    {
		int tBestIndex = 0;
		float tBestDistSq = float.MinValue;
		...
		agent.actionContext.AddContextItem<Vector3>("waypointposition", agent.actionContext.GetContextItem<Vector3>("ratwaypoint" + tBestIndex));
