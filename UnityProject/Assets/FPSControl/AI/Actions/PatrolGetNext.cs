...
public class PatrolGetNext : Action
{
    ...
    public override ActionResult Execute(Agent agent, float deltaTime)
    {
        ...
        agent.actionContext.AddContextItem<int>("patrolnumber", tPatrolNumber);
        agent.actionContext.AddContextItem<Vector3>("patrolposition", agent.actionContext.GetContextItem<Vector3>("patrol" + tPatrolNumber));
